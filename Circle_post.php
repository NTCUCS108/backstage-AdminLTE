...
if(isset($headers) or isset($description) or isset($icon) or isset($link_src) or isset($_SESSION['img_src']))
{
	...
	if(isset($headers) and isset($description) and isset($icon) and isset($link_src) and isset($_SESSION['img_src']))
	{
		...
		if($_GET['use_original_pic']=="false")
		{
			...
			mysql_query("Insert into edit_circle value('$deletepath[0]/delete_img/$deletepath[1]','$rs[headers]','$rs[description]','$rs[icon]','$rs[link_src]','$rs[recent_edit_time]','$rs[post_id]')");
		}
		elseif($_GET['use_original_pic']=="true")
		{
			mysql_query("Insert into edit_circle value('$rs[img_src]','$rs[headers]','$rs[description]','$rs[icon]','$rs[link_src]','$rs[recent_edit_time]','$rs[post_id]')");
		}
		if(isset($_GET['id']))
		{
			$edittime = date("Y-m-d G:i:s");
			mysql_query("update circle set headers = '$headers',description = '$description',icon = '$icon',link_src = '$link_src',img_src = '$_SESSION[img_src]',recent_edit_time = '$edittime' where circle_id = '$id'");
		}
		else
		{
